...
#[test]
fn generic_image_with_exposed_host_port_injects_alias() -> anyhow::Result<()> {
    let _ = pretty_env_logger::try_init();
...
fn exposed_host_ports_are_stored_deduplicated() {
    ...
    assert!(request.hosts().any(
        |(name, host)| name == testcontainers::core::HOST_ACCESS_ALIAS
            && matches!(host, Host::HostGateway)
    ));
}
